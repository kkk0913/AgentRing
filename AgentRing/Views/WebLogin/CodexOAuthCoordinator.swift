...
///
/// 凭据存储约定：refresh_token 存入 `Account.credentialToken`，邮箱或 account_id 存入 `accountIdentifier`。
@MainActor
final class CodexOAuthCoordinator: ObservableObject {
    ...
    private func handleTokens(_ result: Result<CodexOAuthTokens, Error>) {
        ...
        case .success(let tokens):
            ...
            // email 用作去重稳定标识；email 缺失时退回 account_id。
            let account = Account(
                credentialToken: tokens.refreshToken,
                accountIdentifier: email.isEmpty ? (tokens.accountId ?? "") : email,
